...
__global__ void Problem::integrate()
{
    integrateKernel<<<8 * mNumSMs, 64>>>(d_mParticles, mNumParticles,
                                         mGM2, mInverseMass, mDt);

    // Calculate the constant factor for gravitational force
    const double Const = mG * mMass * mMass;

    for (int pi = blockIdx.x * blockDim.x + threadIdx.x;
         pi < numParticles;
         pi += blockDim.x * gridDim.x)
    {
